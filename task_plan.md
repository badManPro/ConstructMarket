...
- 当前已完成的业务实现：
  ...
  - 购物车页：本地购物车读取、勾选/全选、数量步进器、删除、失效商品清空、结算入口
  - 结算页：地址/优惠券/发票容器、备注、支付方式、金额明细、提交订单入口
  - 支付结果页：成功/失败结果态壳子、订单号和金额展示、回首页/购物车
- 当前尚未开始：订单列表/详情本地状态流转、地址/优惠券/发票真实回流页、资讯列表/详情真实实现、客服页真实会话、微信开发者工具联调
- 建议下一步：优先把“交易主链路”补成完整闭环
  - 本地 order mock/store
  - 订单列表页
  - 订单详情页
  - 地址/优惠券/发票页回流
- 当前推荐执行顺序：
  1. 先补本地订单存储和订单状态机，让结算提交不再只停在结果页
  2. 再实现订单列表/详情页，打通支付结果页回流
  3. 再实现地址、优惠券、发票页面与结算页回流
