...
type ArticleCategoryValue = (typeof ARTICLE_CATEGORY)[number];

const articleCategoryMeta: Array<{
...
export const articleFeed: ArticleFeedItem[] = [
  {
    id: "article-01",
...
export function getArticleTabs(): ArticleTab[] {
  return [
    {
      value: "all",
      label: "全部",
      count: articleFeed.length,
      desc: "按时间查看全部资讯内容。",
    },
    ...articleCategoryMeta.map((item) => ({
      value: item.value,
      label: item.label,
      count: articleFeed.filter((article) => article.category === item.value).length,
      desc: item.desc,
    })),
  ];
}
...
export function getArticlesByCategory(category: string) {
  ...
  return articleFeed.filter((item) => item.category === category);
}

export function getArticleById(articleId: string) {
  return articleFeed.find((item) => item.id === articleId) ?? null;
}
...
export function getRelatedArticles(articleId: string) {
  ...
  return current.relatedIds
    ...
    .filter((item): item is ArticleFeedItem => Boolean(item));
}
