...
- 已完成交易链路边界态联调：
  - 结算页新增 Mock 联调开关，支持“提交失败 / 支付成功 / 支付中 / 支付失败”
...
- 已将 Swagger 文档改造成进度看板：
  ...
  - 当前状态拆分：`26` 待接入、`22` 进行中、`1` 前端待补、`9` 后端阻塞
  - 已内置分批推进顺序：A 首页 / 选型 / 搜索结果，B 商品详情 / 收藏加购，C 购物车 / 地址 / 发票基础能力
- 已完成真实接口 B 批代码接入：
  - 商品详情页：真实详情、商户信息、规格分组、SKU 选项、浏览埋点
  - 收藏动作：首页、搜索结果、选型页、商品详情、收藏夹统一经由 `services/browse`
  - 收藏夹页：真实收藏列表、取消收藏、快捷加购
  - 商品详情页与收藏夹页加购：已改走 `POST /v1/app/user/cart`
  - 在线咨询发送：已优先提交 `POST /v1/app/consult-messages`，同时保留本地聊天壳子
- 已再次通过本地校验：
  - `npm run typecheck`
...
当前项目停在“A、B 两批真实接口代码已接入并通过自动化校验，但按当前安排暂时跳过微信开发者工具人工走查，统一标记为未走查；下一步可直接进入 C 批或回头补走查”的阶段。
...
- 已完成交易链路边界态联调：
  - 结算页支持“提交失败 / 支付成功 / 支付中 / 支付失败”联调切换
  - 提交订单后先写入 `pending_payment` 订单，再由支付结果页更新支付态
  - 支付结果页已补齐三态切换与失败重试
  - 订单列表页、订单详情页待支付订单统一走支付结果页
  - 购物车、结算、订单列表、订单详情支持 `loading / error / offline` 演示态
- 已完成真实接口 B 批代码接入：
  - 商品详情页已切 `product/detail + product/specs + merchant/detail`
  - 商品详情、首页、搜索结果、选型页、收藏夹的收藏动作已统一切真实收藏接口
  - 商品详情加购与收藏夹快捷加购已统一切真实购物车接口
  - 商品详情进入时已补记真实浏览记录
  - 在线咨询页发送消息已优先切真实留言接口
- 当前 A、B 批状态：
  - 代码和自动化验证已完成
...
优先做 `C. 购物车 / 地址 / 发票基础能力`，并把 A、B 批继续保留为“未走查”：

1. 继续保证后续改动遵循 `docs/ui-reference-guideline.md`
2. 按 [`docs/swagger-app-接口映射.md`](./swagger-app-接口映射.md) 推进 C 批
3. 在合适时间补做 A、B 批微信开发者工具走查，并将结果回写 `progress.md` 与 Swagger 看板

推荐顺序：

1. 继续保证后续改动遵循 `docs/ui-reference-guideline.md`
2. 继续真实接口对接，下一批进入 `C. 购物车 / 地址 / 发票基础能力`
3. 在 A、B、C 任一批准备改成 `已完成` 前，再补做对应 DevTools 人工走查
